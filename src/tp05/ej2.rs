...
use std::{clone, collections::VecDeque, fs::File, io::Write};
...
impl Playlist {
    fn agregar_cancion(&mut self, cancion: Cancion) {
        ...
        let path = "src/tp05/ej2.json";
        let mut archivo = File::create(path).unwrap();
        let canciones_serializadas = serde_json::to_string(&self.canciones).unwrap();
        archivo
            .write_all(canciones_serializadas.as_bytes())
            .unwrap();
    }
